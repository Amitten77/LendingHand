...
contract LendingHand {
   ...
   function createPost(string memory _name, string memory _description, uint _goal) public {
        ...
        postCount += 1;
        posts[postCount] = Post(postCount, _name, _description, _goal, 0, msg.sender, false, _donors, _donations, 0); //msg.sender is person who called the function
        //Trigger an event (debugging purposes)
        emit PostCreated(postCount, _name, _description, _goal, 0, msg.sender, false, _donors, _donations, 0);
   }
   ...
   function donateToPost(uint _id) public payable {
    ...
    //Trigger an event
    emit PostDonated(_id, _post.name, _post.description, _post.goal, _post.current, _donee, _post.reachedGoal, _post.donors, _post.donations, _post.numDonors);
   }
